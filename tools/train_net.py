...
def train(cfg, local_rank, distributed):
	model = build_detection_model(cfg) # build model here (not loaded yet)
	...
	# load weight for normal resnet
	if cfg.MODEL.BACKBONE.CONV_BODY == "R-50-NORMAL-C4":
		resnet = models.resnet50()
		resnet_state_dict = torch.load("./data/models/resnet50_IN.pth")
		resnet.load_state_dict(resnet_state_dict)
		resnet.to(device)

		for k, v in model._modules.items():
			if k == "backbone":
				for l, w in v._modules.items():
					for m, x in  w._modules.items():
						if m in resnet._modules:
							model._modules[k]._modules[l]._modules[m] = copy.deepcopy(resnet._modules[m])
